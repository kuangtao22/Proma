...
- 2026-08-30：Agent Canvas 批量提交由主进程唯一服务与 LOAD/SAVE/单节点操作共享同一按 Canvas serializer 和 workspace write lease；`canvas-batch-<UUID>.json` 对每项 Agent session、内容目录及内容 trash 移动先持久化 `pending/preparing/ready/cleanup-pending/cleaned` ownership，再执行副作用，清理失败或目录耐久不确定时保留恢复证据并要求 LOAD 对账。图片、文档、原型删除复用现有 `CanvasTrashEntry` move/restore 生命周期，图只单次 `mutate`，Agent 删除继续保留旧 session；Store mutation 校验以节点/边 Map、Set 和邻接索引保持 O(n)，`sourceToolCallId` 作为跨重试幂等键。
