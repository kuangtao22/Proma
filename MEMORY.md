...
- 2026-08-30：Agent Canvas 多节点与连线批量提交使用目标 Canvas 内 `canvas-batch-<UUID>.json` 的 `prepared -> resources-created -> committed` intent；完整 mutation 必须先由 CanvasDocumentStore 权威验证，再在同一 workspace write lease 和按 Canvas 串行边界内只调用一次 `mutate`。Agent session 与内容目录由现有生命周期窄 API 准备，提交前失败只逆序回收本轮新建资源；图事实已可见或 durability 不确定时禁止回滚，后续 LOAD 依据 intent 与单 revision 事实补齐 committed，`sourceToolCallId` 作为跨重试幂等键。
