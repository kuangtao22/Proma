...
- 2026-08-29：Agent-Canvas 共享合同固定以 `AgentCanvasBinding` 维护项目内会话关联，重复画布按首现顺序去重且默认/最近画布必须已关联；发送与队列使用公开 `canvasNodeReferences`，SDK JSONL 持久化使用私有 `_canvasNodeReferences`，两者不复用附件或 mention。共享层严格解析节点引用与批量命令外壳；真实 Agent 发送边界由 Agent service 接入引用解析，批量 mutation 的完整 schema、重复 ID 和图引用由事务执行前的 CanvasDocumentStore 权威验证，禁止在共享类型重复维护图语义。
