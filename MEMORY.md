...
- 2026-08-29：Agent-Canvas LIST 的 legacy 投影与失效关联修复属于项目 mutation，必须先检查只读原因并进入 workspace write guard；画布枚举保持纯读。项目级失效会话与画布对账由 Store 单次 fresh-read、单次 secure CAS 提交，只有提交成功后统一广播；CAS 冲突轮零事件并由下一次 LIST 基于磁盘权威重试。CAS 成功后不得再次读取易变会话事实来阻断已提交 changes 的广播，返回与事件都使用同轮对账快照。
