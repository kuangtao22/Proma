...
- 2026-08-29：普通 Agent 与项目画布的多对多关联独立保存在业务配置根 `agent-canvas-bindings.json`（schema v1），不写入 `AgentSessionMeta` 或画布文档；索引严格拒绝未知字段、错误版本和重复项目+会话身份，损坏读取只降级为空并保留原文件，直到下一次有效业务变更才通过 safe-file 原子重建，no-op 不写盘。
