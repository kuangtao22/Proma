...
- 2026-08-30：Agent Canvas 批量提交由主进程唯一服务与 LOAD/SAVE/所有可能写图的单节点及 Agent 对账操作共享同一按 Canvas serializer 和 workspace write lease；任何新批次或其它图变更都必须先收敛该 Canvas 的全部 pending batch intent，恢复失败时禁止产生第二份 intent 或当前操作副作用，恢复 publication 在 lease 释放后先于当前操作发布。`canvas-batch-<UUID>.json` 对每项 Agent session、内容目录及内容 trash 移动先持久化 `pending/preparing/ready/cleanup-pending/cleaned` ownership，再执行副作用；Store 从权威基线按 mutation 顺序规划一次最终文档，intent 固化不含 revision/时间的最终图 SHA-256，不确定提交只接受 `baseRevision + 1` 与精确哈希同时匹配。资源按基线到最终图的净差异准备，create→remove 不创建目录、trash 或 session，remove→recreate 同 ID 在副作用前拒绝；图片、文档、原型删除继续复用 `CanvasTrashEntry` move/restore，Agent 删除保留旧 session。清理失败或目录耐久不确定时保留恢复证据并要求 LOAD 对账；Store mutation 校验以节点/边 Map、Set 和邻接索引保持 O(n)，`sourceToolCallId` 作为跨重试幂等键。
