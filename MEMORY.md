...
- 2026-08-23：设计素材导入遇到 `DESIGN_RECOVERY_REQUIRED` 时，Inspector 仅通过项目级一次性 Jotai 请求通知画布，唯一工作区 controller 强制加载权威快照并清空历史、选区、待保存队列与冲突状态，同时接管新媒体授权 URL；恢复完成后允许用户再次导入。
