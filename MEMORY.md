...
- 2026-09-01：Canvas 开放能力新增 Proma 内置 `canvas-production` Skill：Skill 只负责用户意图触发、产物路由和七个 Canvas 工具的操作编排；项目授权、Schema、revision、事务、路径、安全边界、删除/覆盖/付费审批继续由原生 Host 负责。Skill 的启停不得成为权限依据，Canvas 工具仍原生注入；长流程说明从 `systemPromptAppend` 迁入 Skill，原生提示仅保留最小安全合同。为什么这样处理：工作流知识需要独立迭代和按需加载，但可编辑、可停用的 Skill 不能承担可信执行。对用户的影响是 Canvas 请求编排更一致，普通代码任务减少无关提示和误路由，停用 Skill 不会削弱安全边界或删除既有产物。
...
- 2026-09-01：Canvas 四类节点统一使用结构化 `activityState` 表达活动状态：实际 `running` 才显示沿卡片外圈流动的虚线，`queued` 与 `waiting-approval` 只显示静态轮廓，候选待验收改用“有新版本”标记。卡片不得从中文状态文案反推运行态；动画层不改变几何、不接管指针，并遵循减少动态效果。为什么这样处理：运行、排队、审批和候选是不同事实，统一转动会误导资源是否正在消耗；对用户的影响是缩放画布后仍能快速识别真实执行节点。性能上只为活动节点挂载 CSS 驱动的 SVG 轮廓，不增加定时器或 I/O。
