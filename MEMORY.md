...
- 2026-08-26：Canvas Agent SEND 的 Renderer 乐观运行与 bootstrap/流事件权威运行必须分层；`userMessageUuid` 同时作为乐观 generation，迟到 reject 只能收口匹配 token。主进程同会话 busy 通过稳定内部码映射为公开 `SESSION_BUSY`，保留真实 running/owner 以维持 STOP 可用和 SEND 禁用；hard invalid 必须立即回收 live/error/stream 共享状态，soft invalid 继续保留 active fail-closed runtime。
