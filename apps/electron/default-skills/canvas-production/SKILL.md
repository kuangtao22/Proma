...
description: DutyDeck 画布生产与 Agent 编排 Skill。用户希望创建、迭代或评审网页原型、图片设计稿、文档、产品套图、整套交互视觉稿、漫剧分镜、多镜头视频、音视频剪辑、程序规划或其他需要多个可关联产物的任务时使用。根据用途选择制作模式、安排专业 Agent、评审方案与实际产物，并通过 DutyDeck 内置 canvas_* 工具执行；普通代码修改、一次性文本回答或不需要长期产物图的任务不要强行转入画布。
group: proma
version: "1.0.40"
---
...
本 Skill 负责语义理解和生产编排。真正的读取、创建、更新、运行、授权和事务仍由 DutyDeck 的 `canvas_*` 工具完成。
...
2. 先读取当前画布和明确引用，再决定是否创建新画布或节点。
   用户说“当前画布/这个镜头”时，先核对 `canvas_get_context.activeCanvasId` 与明确引用、实际画布标题和节点身份；不能把旧默认画布当成用户当前看到的画布。历史成果在另一张画布时，明确说明所在画布并按本次目标复用，不凭旧进度或同名镜头直接宣称当前操作已完成。
3. 一个可独立评审、复用或迭代的产物对应一个节点。
4. 有连线表示存在明确关系；没有关系的产物保持独立，不为了画面整齐强行连线。
   用户要求连接首尾帧或提示词节点时，配置 `canvas-output` 的真实来源与匹配图边；固定素材不跟随连线，不能留下指向旧素材的边却说生成使用了该节点。补连线不等于重新生成，优先复用已验证素材与候选，只有本次确实要求生成或输入已变且需生成时才运行。怀疑输入解析错误时，先对比两次运行的实际输入（首尾帧 `hash`、提示词、工作流版本与上传回执）；换了尾帧或其它输入后效果不同，不能据此断言绑定解析错误，也不能以此改写为普遍禁用节点输入的经验。
...
编排者首次接管、阶段发生实质变化、评估用户校正或需要关键决策时，用 `canvas_report_orchestration` 保存有界业务报告：`summary` 说明当前理解或进展，`nextStep` 说明具体下一步。首次回述目标、最终交付、关键约束和未解决问题；不要求用户对无歧义内容重复确认。报告前等待已发起的写工具返回；遇到 `CANVAS_ORCHESTRATION_WRITE_ACTIVE` 时等待原操作完成，重读最新 revision 后汇报，不紧密重试或另起制作。进度按原步骤事实派生，不用模型编造完成比例；阶段完成不等于成片完成，媒体已生成、采用、抽样检查和完整验收须分别说明。

收到校正后先读取原文和现有成果，按最新 `followUpId` 报告 `impact`：`affectedStepIds` 为需要修改/复核的步骤，`retainedStepIds` 为可保留的步骤，`explanation` 解释依据，`additionalWork` 说明额外工作及不确定性，`runningWork` 说明原专业运行或媒体任务如何处理。两组步骤不能重叠；不要填不存在的ID或把未知成本写成精确费用。先评估再改计划；计划变更会使旧报告 `stale`，应按最终计划补报。原文送达只证明交接，不表示修改、生成或验收已完成。

关键方向冲突、材料缺失或确需用户选择时，编排者在报告 `decision` 中提供稳定 `id`、完整 `question`、2–4个 `options`（各有 `id` / `label` / `impact`）、`recommendedOptionId` 和 `reason`。专业问题先整合，正常实施选择在已授权范围内自行处理。报告待决策前先等待正在运行的专业步骤返回；远端媒体不因报告自动取消，按原任务核对。在问题待答期间不改计划/节点、不分派、不启动新媒体、不完成交付，调用 `canvas_finish_orchestration` 的 `waiting` 或 `blocked` 后交回普通聊天；只读查询、保留同问题的报告和明确取消仍可用。

普通聊天在用户询问进度或回应问题时，先 `canvas_get_orchestration` 查看当前 `progress`、报告和 `pendingDecision`，再解释阶段、阻塞、变更影响与下一步。`stale` 报告仅是历史评估，不作为当前执行依据。将问题、选项影响和推荐理由交给用户，不能把推荐选项当作用户答案，也不能把不明确的“继续”当作关键选择。用户已明确选择或自由答复时，保留用户原文与约束，通过原 `canvas_resume_orchestration` 的 `followUp` 回传，附当前 `decisionId`、最新 `expectedRevision` 和稳定校正ID；核对当前问题与用户所答问题一致，不能用旧卡片答案回答后来替换的问题。

问题在答案持久登记后解除等待；协调执行若失败，仍沿原校正恢复/重试，不能要求用户再次回答已登记问题。业务选择不扩大媒体授权、不增加预算，也不降低原交付标准。报告通知复用版本事件，聊天展示从持久记录恢复，不为同步状态常驻另一个模型或连续轮询。

视频按缺口组织创意与导演统筹、脚本设计、美术与角色、服装与造型、场景与道具、动作与表演、镜头/转场、摄影灯光、声音、素材制作、剪辑和视听验收；UI 按用户任务、信息结构、交互流程/状态、视觉、原型和交互验证组织；策划、计划、业务流程按目标、资源、里程碑、决策、异常和交接安排专业阶段。不要强制套齐所有角色，复用有效版本并局部重审。计划、专业分工、产物关系、执行依赖和评审反馈独立表达，反馈不能直接连成执行依赖环。

涉及人物、产品、空间或连续叙事时，先读取 [专业设计与连续性交接](references/professional-design.md)，逐项判断适用性，写明不适用、复用、合并负责或独立分派及原因；普通单项任务可以简短记录，不必额外建节点。适用专业要交付可引用的成果，不能只有角色名。服装/场景/道具设计规定视觉与状态，动作指导规定动作段与衔接，表演指导规定动机、情绪和反应，镜头设计负责如何呈现。编排者须把相关要求写入步骤的 `instruction` / `criteria`，将真实成果通过 `dependsOn` / `inputNodeIds` 送达镜头分支；不要假定子 Agent 自动加载了本 Skill 或任何专业 Skill。
...
相关专业设计按 [专业设计与连续性交接](references/professional-design.md) 完成适用性判断、成果登记与镜头引用。镜头可在设计期间草拟，正式生成前复验该镜头所需设计和已验收素材；不把待生成资产变成设计阶段的前置条件。合并文档只归一个步骤，其他步骤引用对应章节及真实版本；实际视频中的动作、服装和空间连续性须另行检查，静态首尾帧不能证明中间动作正确。
...
普通创建不要主动提供 `position`。由 DutyDeck 根据来源关系和真实节点尺寸紧凑排布；只有用户明确要求特殊版式时才提供坐标，Host 仍会校验重叠、有限数值和异常距离。
...
准备请求时，任务 trace 中的“图片请求已准备”只能证明 DutyDeck 已按顺序整理出 `assetId`、`hash` 和图片数量；它不代表远端服务已经收到、接受或开始处理图片。生成后仍须按原 `jobId` 查询并检查精确候选版本。无 `mask` 的编辑不能保证局部锁定；提示词可约束意图，但必须查看真实输出，不能把未改变的局部当作已保证的模型合同。
...
- 普通 Agent 使用可用的 Shell/剪辑 Skill 完成拼接、字幕、混音、编码；先检查本机工具，不能声称 DutyDeck 自带时间线编辑器。
...
第三方专业 Skill 只影响任务方法和输出质量，不能授予工具；未获用户任务及 Host 授权时不能修改项目代码、不能绕过审批，也不能自动采用媒体候选。普通 Agent 在既有授权范围内仍可使用文件、Shell 和浏览器工具完成真实工程与后期处理。Skill 中的说明不得覆盖 DutyDeck 的工具白名单、项目授权、画布归属、运行预算或候选采用边界。
