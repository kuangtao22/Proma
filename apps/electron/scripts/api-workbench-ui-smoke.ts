 ...
 * 打开目录行的「…」操作菜单。
 *
 * Radix 的菜单触发器只认指针事件，直接 .click() 不会展开，因此这里合成 pointerdown/pointerup。
 *
 * @param kind 行类型：集合一级行或分组行
 * @param name 行名称（集合名或分组名），用于在正确的行上定位触发器
 */
async function openCatalogRowMenu(window: BrowserWindow, kind: 'collection' | 'folder', name: string): Promise<void> {
  const rowSelector = kind === 'collection' ? '[data-api-collection-header]' : '[data-api-folder-header]'
  const labelPrefix = kind === 'collection' ? '集合操作 ' : '分组操作 '
  const pressed = await window.webContents.executeJavaScript(`(() => {
    ...
    const trigger = [...(row?.querySelectorAll('[aria-label]') ?? [])].find((node) => (node.getAttribute('aria-label') ?? '').startsWith(${JSON.stringify(labelPrefix)}))
    if (!(trigger instanceof HTMLElement)) return false
    const init = { bubbles: true, cancelable: true, pointerType: 'mouse', button: 0, buttons: 1 }
    trigger.dispatchEvent(new PointerEvent('pointerdown', init))
    trigger.dispatchEvent(new PointerEvent('pointerup', { ...init, buttons: 0 }))
    return true
  })()`)
  assert.equal(pressed, true, `找不到${kind === 'collection' ? '集合' : '分组'}「${name}」的「…」入口`)
  await waitFor(window, "Boolean(document.querySelector('[role=menuitem]'))", `「${name}」的操作菜单未打开`)
}

/** 选择已打开的「…」菜单项（同样只认指针事件）。 */
async function chooseCatalogMenuItem(window: BrowserWindow, text: string): Promise<void> {
  await waitFor(window, `Boolean([...document.querySelectorAll('[role=menuitem]')].find((item) => item.textContent?.trim() === ${JSON.stringify(text)}))`, `菜单里没有「${text}」`)
  const pressed = await window.webContents.executeJavaScript(`(() => {
    const item = [...document.querySelectorAll('[role=menuitem]')].find((node) => node.textContent?.trim() === ${JSON.stringify(text)})
  ...
  assert.equal(pressed, true, `无法点击菜单项「${text}」`)
...
async function runElectronSmoke(app: import('electron').App, BrowserWindow: typeof import('electron').BrowserWindow, userData: string): Promise<void> {
  ...
  try {
    ...
    const extractionCollection = await window.webContents.executeJavaScript(`(() => {
      const section = [...document.querySelectorAll('section')].find((item) => item.textContent?.includes('/imported'))
      return section?.querySelector('[data-api-collection-header]')?.textContent?.trim() ?? null
    })()`) as string | null
    assert.ok(extractionCollection, '找不到包含导入请求的集合')
    await openCatalogRowMenu(window, 'collection', extractionCollection)
    await chooseCatalogMenuItem(window, '主机提取为变量')
    ...
    try {
      ...
      await writeFile('/private/tmp/api-workbench-ui-collection-menu.png', (await stickyWindow.webContents.capturePage()).toPNG())
      ...
      const clickFolderToggle = async (prefix: string): Promise<void> => {
        const clicked = await stickyWindow.webContents.executeJavaScript(`(() => {
          const header = document.querySelector('[data-api-folder-header]')
          const toggle = [...(header?.querySelectorAll('button[aria-label]') ?? [])].find((node) => (node.getAttribute('aria-label') ?? '').startsWith(${JSON.stringify(prefix)}))
          if (!(toggle instanceof HTMLButtonElement)) return false
          toggle.click()
          return true
        })()`)
        assert.equal(clicked, true, `找不到分组「${prefix}」入口`)
      }
      await clickFolderToggle('收起分组')
      await waitFor(stickyWindow, "document.querySelectorAll('[data-api-request-id]').length === 30", '收起分组后分组的请求没有被隐藏')
      /** 只数条数不够：必须确认剩下的都是根目录请求，分组的请求真的不再渲染。 */
      const collapsedRows = await stickyWindow.webContents.executeJavaScript("[...document.querySelectorAll('[data-api-request-id]')].map((node) => node.getAttribute('data-api-request-id'))") as string[]
      assert.equal(collapsedRows.length, 30, `收起分组后可见请求应为 30 条（当前 ${collapsedRows.length}）`)
      const leftoverFolderRows = collapsedRows.filter((id) => Number(id.split('_')[1]) % 2 === 0)
      assert.equal(leftoverFolderRows.length, 0, `收起分组后仍渲染了该分组的请求：${leftoverFolderRows.slice(0, 5).join(', ')}`)
      await writeFile('/private/tmp/api-workbench-ui-folder-collapsed.png', (await stickyWindow.webContents.capturePage()).toPNG())
      await clickFolderToggle('展开分组')
      await waitFor(stickyWindow, "document.querySelectorAll('[data-api-request-id]').length === 60", '展开分组后请求没有恢复')
      /** 搜索必须强制展开：命中的请求不能被分组折叠状态藏起来。 */
      await clickFolderToggle('收起分组')
      await waitFor(stickyWindow, "document.querySelectorAll('[data-api-request-id]').length === 30", '第二次收起分组失败')
      await fill(stickyWindow, 'input[aria-label="搜索请求"]', '渠道模型绑定 2')
