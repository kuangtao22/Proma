...
import { createCanvasWorkflowAgentRecovery, createCanvasWorkflowMediaAdapter } from './lib/design/canvas-workflow-runtime-adapters'
...
/** 延续主分支启动修复：数据根准备完成后才创建唯一关联 Store。 */
let sharedAgentCanvasBindingStore: AgentCanvasBindingStore | null = null

/** 普通 IPC 注册时取得唯一缓存与 CAS 所有者，模块导入不解析业务数据根。 */
function getAgentCanvasBindingStore(): AgentCanvasBindingStore {
  if (sharedAgentCanvasBindingStore === null) sharedAgentCanvasBindingStore = new AgentCanvasBindingStore()
...
export function registerIpcHandlers(): void {
  ...
  const getDesignProjectReadOnlyReason = (projectId: string): string | undefined => {
    ...
    /** 外部项目不可访问时禁止创建同名替代目录。 */
    const rootStatus = getLocalProjectRootStatusSync(workspace.projectRootPath)
  ...
  const canvasImageCandidateBatchService = createCanvasImageCandidateBatchService({
    ...
    /** 定向重试入口会在同一 Canvas 锁内复核节点仍绑定当前图片模块。 */
    assertRetryTarget: async (target) => canvasImageJobTarget.assertTarget(target.projectId, {
      kind: 'canvas-image',
      canvasId: target.canvasId,
  ...
  const canvasWorkflowExecutionService = createCanvasWorkflowExecutionService({
    ...
    recoverAgentExecution: createCanvasWorkflowAgentRecovery({
      load: (target) => canvasDocumentStore.load(target).document,
      outputs: canvasAgentOutputService,
    ...
    isImageCandidateAdopted: async (query) => {
      const batch = await canvasImageCandidateBatchService.load(query)
      const entry = batch.entries.find((item) => item.nodeId === query.nodeId && item.jobId === query.taskId)
      const node = canvasDocumentStore.load(query).document.nodes.find((item) => item.id === query.nodeId)
      const adopted = entry?.status === 'adopted' && !!entry.candidateAssetId && node?.kind === 'image'
        && node.adoptedAssetId === entry.candidateAssetId
      const asset = adopted ? designStore.requireStableAuthoritativeDocument(query.projectId).assets.find((item) => item.id === entry.candidateAssetId) : undefined
      return { adopted: !!asset && !!batch.adoption, artifactHash: asset?.sha256 ?? null, committedAt: batch.adoption?.committedAt ?? null }
    },
  ...
  ipcMain.handle(
    ...
    async (_, id: string, projectRootPath: string): Promise<AgentWorkspace> => {
      return workspaceOperationGuard.runWorkspaceWrite(id, async () => {
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, name: string): Promise<WorkspaceMcpConfig> => {
      return workspaceOperationGuard.runWorkspaceSlugWrite(workspaceSlug, () => {
        const current = getWorkspaceMcpConfig(workspaceSlug)
