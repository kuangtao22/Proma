...
describe('Pi Server Ops 多资源只读工具', () => {
  test('Given Facade 已绑定真实会话 When 构建工具 Then 旧读取工具不扩参且 SQL 使用独立工具', () => {
    ...
    expect(tools.map((tool) => tool.name)).toEqual([
      'ops_resources', 'ops_server_overview', 'ops_server_services', 'ops_data_test',
      'ops_data_diagnose', 'ops_database_tables', 'ops_database_describe', 'ops_database_rows', 'ops_database_query',
    ])
