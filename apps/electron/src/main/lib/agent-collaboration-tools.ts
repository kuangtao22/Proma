...
export function buildPiCollaborationTools(
  ...
  return [
    ...
    sdk.defineTool({
      ...
      label: '委派子 Agent',
      description: '创建一个真实可见的 DutyDeck 协作子 Agent 会话来并行处理独立子任务。可选 thinkingLevel 指定子会话首轮思考强度；模型不支持时运行时会安全归一化。返回中的 configuredThinkingLevel/thinkingLevel 表示配置/请求值，不代表模型 capability normalization 后的实际运行档位。委派只表示子会话已启动。',
      parameters: Type.Object({
        title: Type.Optional(Type.String({ description: '子会话标题' })),
        role: roleType,
    ...
    sdk.defineTool({
      ...
      label: '批量委派子 Agent',
      description: '批量创建多个真实可见的 DutyDeck 协作子 Agent 会话。每个 item 可独立指定首轮 thinkingLevel；模型不支持时运行时会安全归一化。返回中的 configuredThinkingLevels/delegations[].thinkingLevel 表示各项配置/请求值，不代表模型 capability normalization 后的实际运行档位。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '等待子会话完成',
      description: '父会话用于收敛子会话结果的等待屏障：等待指定的 DutyDeck 协作子会话完成，并返回结构化结果摘要。只要本轮回复、决策或交付依赖已委派任务，主会话必须在回复前调用本工具，不能只因 delegate_agent/delegate_agents 已返回就宣称完成；需要全部结果时传入所有 delegationIds 并使用 mode=all，需要部分早期结果时才使用 mode=any。若返回 timeout 或仍有 running 委派，必须如实说明未收敛状态，不能把未完成任务当作已有结果。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '列出协作子会话',
      description: '列出当前父会话创建的 DutyDeck 协作子会话及状态。返回中的 thinkingLevel 表示配置/请求值，不代表模型 capability normalization 后的实际运行档位。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '读取子会话结果',
      description: '按委派 ID 读取一个或多个 DutyDeck 协作子会话的结果摘要。返回中的 thinkingLevel 表示配置/请求值，不代表模型 capability normalization 后的实际运行档位。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '停止子会话',
      description: '停止一个正在运行的 DutyDeck 协作子会话。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '批量停止子会话',
      description: '批量停止多个正在运行的 DutyDeck 协作子会话。',
      parameters: Type.Object({
