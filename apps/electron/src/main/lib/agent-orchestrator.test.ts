...
describe('Agent sendMessage 准入顺序合同', () => {
  ...
  test('Given 单次运行注入可信生图路由 When 构建 Pi 工具 Then 仅经运行扩展传入内置工具上下文', () => {
    ...
    expect(body).toContain('trustedImagePrompt: extensions.trustedImagePrompt')
    expect(body).toContain('trustedImageParameters: extensions.trustedImageParameters')
    expect(body).toContain('trustedReferenceImagePaths: extensions.trustedReferenceImagePaths')
    expect(body).toContain('captureDesignImageRequest: extensions.captureDesignImageRequest')
  ...
  test('Given bypass Canvas 运行工具 When 进入权限边界 Then 守卫后逐次审批且普通 Canvas 工具不审批', () => {
    ...
    expect(body.slice(approvalIndex, bypassIndex)).toContain("extensions.toolApprovalPolicy?.getMode(toolName, input) === 'automatic'")
