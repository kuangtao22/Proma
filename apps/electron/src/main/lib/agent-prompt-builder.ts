...
export function buildSystemPrompt(ctx: SystemPromptContext): string {
  ...
  const productivityTools: ProductivityToolsSettings = ctx.productivityTools ?? {
    todosEnabled: true,
    calendarEnabled: true,
    obsidianEnabled: true,
  }
  const planningPrompt = productivityTools.todosEnabled || productivityTools.calendarEnabled
    ? [
        '## 任务、日程与自动化',
        productivityTools.todosEnabled ? '明确且用户认可的后续行动用 Todo；创建 Todo 前必须调用 `list_todos({ status: \'open\', limit: 100 })` 与 `list_groups({ scope: \'todo\' })` 去重/复用；已有事项只按事实更新或完成，取消不删除。' : '',
        productivityTools.calendarEnabled ? '有明确开始时间的安排用日程。' : '',
  ...
  const vaultPrompt = productivityTools.obsidianEnabled
    ? '## Vault\n\n- 当用户在会话右侧打开 Vault 标签、要求查找、阅读、整理或编辑 Obsidian 笔记时，使用 Vault 工作流。\n- Vault 保留为普通 Markdown 文件；先读取目标文件和相关上下文，再做小范围修改。'
    : undefined
...
- 一项操作确定需要可见终端时，**优先复用而非新开 Tab**：先用 \`TerminalList\` 查看本会话终端，选择 cwd 一致、仍在运行且你已观察到上一条命令结束的终端，并在 \`TerminalExecute\` 中传入 \`terminalId\`。仅在没有这种安全候选、cwd 或 shell 必须改变、或需要让用户独立观察并行会话时，才新开终端。交互式、长驻或忙碌状态不明的终端不可复用；需要确认完成状态或命令结果时使用 \`TerminalRead\`。`,
    ...
    ctx.collaborationAvailable
      ...
      : undefined,
    `## 视觉设计、画布与代码实现
- 用户提出“设计首页、做视觉、生成效果图、画一版界面”等要求时，先判断目标是画布中的可持续产物，还是直接修改代码实现；不要把“设计”默认解释为开发。
- 用户明确要求使用画布，或任务明显需要多个可关联、可持续迭代的产物时，由当前普通 Agent 加载并遵循 \`canvas-production\` Skill，使用 \`canvas_*\` 工具完成画布选择、产物创建、关系编排和按需运行。
- 不得要求用户切换到另一个 Design/Canvas，也不得把任务转交给不存在的独立 Design 面板；当前会话负责与右侧画布工作区协同。
- 用户需求不足以判断最终交付是画布产物还是代码实现，且该差异会实质影响结果时，只澄清期望的交付形态；用户明确要求实现、开发或修改代码时，进入正常开发流程。
