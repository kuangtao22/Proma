...
describe('Canvas Workflow Run Store', () => {
  ...
  test('Given 不同父 Agent 代次复用 operation When 创建 Then 分配独立运行身份且可精确查回', () => {
    const fixture = createFixture()
    const store = fixture.createStore()
    const first = store.create(fixture.input)
    const secondOwner = { sessionId: 'session-2', runStartedAt: 20 }
    const second = store.create({ ...fixture.input, owner: secondOwner })

    expect(second.id).not.toBe(first.id)
    expect(second.id).toBe(createCanvasWorkflowRunId(
      fixture.input.projectId,
      fixture.input.canvasId,
      fixture.input.operationId,
      secondOwner,
    ))
    expect(store.findByOperation(fixture.input, fixture.input.operationId, fixture.input.owner)?.id)
      .toBe(first.id)
    expect(store.findByOperation(fixture.input, fixture.input.operationId, secondOwner)?.id)
      .toBe(second.id)
  })

  test('Given 同一运行已有推进者 When 竞争跨进程租约 Then 只允许一个持有者', () => {
    const fixture = createFixture()
    const store = fixture.createStore()
    const run = store.create(fixture.input)
    const release = store.acquireLease(fixture.input, run.id)

    expect(release).toBeFunction()
    expect(store.acquireLease(fixture.input, run.id)).toBeNull()
    release?.()
    const reacquired = store.acquireLease(fixture.input, run.id)
    expect(reacquired).toBeFunction()
    reacquired?.()
  })

  test('Given 单 Canvas 超过 512 条历史 When 分页读取 Then 全部运行仍按稳定顺序可达', () => {
    const fixture = createFixture()
    let timestamp = 100
    const store = fixture.createStore(() => timestamp += 1)
    ...
    do {
      const page = store.listPage(fixture.input, { cursor, limit: 128 })
  ...
  test('Given 其它 Agent 占满较新页 When 按 owner 分页 Then 当前 Agent 历史不会被挤掉', () => {
    const fixture = createFixture()
    let timestamp = 100
    const store = fixture.createStore(() => timestamp += 1)
    for (let index = 0; index < 20; index += 1) {
      store.create({ ...fixture.input, operationId: `mine-${index}` })
    }
    for (let index = 0; index < 270; index += 1) {
      store.create({
        ...fixture.input,
        operationId: `other-${index}`,
        owner: { sessionId: 'other-session', runStartedAt: index + 1 },
      })
    }

    const page = store.listPage(fixture.input, { ownerSessionId: 'session-1', limit: 256 })
    expect(page.runs).toHaveLength(20)
    expect(page.runs.every((run) => run.owner.sessionId === 'session-1')).toBe(true)
