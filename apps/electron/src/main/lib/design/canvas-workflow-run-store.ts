  ...
  const store: CanvasWorkflowRunStore = {
    create(input) {
      const result = dependencies.runWorkspaceWrite(input.projectId, () => {
        ...
        /** 所有同 operation 创建者共用旧 ID 锁，保证 owner 分流只有一个决定者。 */
        const release = acquireMediaFileLock(join(runDirectory, `workflow-run-${legacyRunId}.lock`))
        try {
    ...
    findByOperation(target, operationId, owner) {
      ...
      try {
        const legacy = store.get(target, runId)
        if (!owner || isSameOwner(legacy.owner, owner)) return legacy
      } catch (error) {
    ...
    listPage(target, options = {}) {
      ...
      const runs = candidates.slice(0, limit).map((run) => structuredClone(run))
      return {
        runs,
        nextCursor: candidates.length > limit && runs.length > 0
          ? encodeCursor(runs[runs.length - 1]!)
          : null,
      }
    },
    ...
    acquireLease(target, runId) {
      if (!/^[a-f0-9]{48}$/.test(runId)) throw new Error('CANVAS_WORKFLOW_RUN_ID_INVALID')
      try {
        return acquireMediaFileLock(join(directory(target, true), `workflow-run-${runId}.drive.lock`))
      } catch (error) {
        if (error instanceof Error && error.message === 'MEDIA_FILE_BUSY') return null
        throw error
      }
    },
