...
function profile(
  ...
  const common = {
    ...
    models: [{ id: `${provider}-model`, voices: [{ id: `${provider}-voice`, name: `${provider}-voice`, source: 'manual' as const }] }],
    enabled: true,
    createdAt: 10,
    updatedAt: 20,
  }
...
describe('独立音频供应商测试服务', () => {
  ...
  test('Given Store 查找或解密失败 When 测试已保存配置 Then 只返回固定 failed', async () => {
    for (const store of [
      storeFixture({ publicCatalog: { schemaVersion: 3, revision: 0, profiles: [] } }),
