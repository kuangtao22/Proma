...
import { getServerOpsAuditErrorCode, ServerOpsAuditStore, sanitizeServerOpsAuditCommand } from './server-ops-audit-store'
...
describe('Server Ops Agent 审计 Store', () => {
  ...
  test('Given 可写空 Store When 追加超过上限 Then 固定保留最近 5000 条并返回深拷贝', () => {
    ...
    let now = 1
    let uuid = 0
    /** 捕获最后一次原子写入内容，避免 rotation 单元测试重复写入大型临时文件。 */
    let written: object | undefined
    const store = new ServerOpsAuditStore(configDir, {
      now: () => now++,
      uuid: () => `audit-${++uuid}`,
      writeJson: (_filePath, data) => { written = data },
    })
    for (let index = 0; index < 5_002; index += 1) {
      store.append({ actor: 'agent', sessionId: 'session-1', hostId: 'host-1', operation: 'exec', phase: 'start', outcome: 'success', command: `echo ${index}` })
    }

    const records = store.list({ limit: 5_000 }).records
    expect(records).toHaveLength(5_000)
    expect(records[0]?.id).toBe('audit-3')
  ...
  test('Given 默认写入依赖 When 追加记录 Then 使用版本 2 原子 JSON 文件', () => {
    const configDir = createConfigDir()
    const store = new ServerOpsAuditStore(configDir, { uuid: () => 'audit-1', now: () => 1 })

    store.append({ actor: 'agent', sessionId: 'session-1', hostId: 'host-1', operation: 'connect', phase: 'start', outcome: 'success' })

    expect(JSON.parse(readFileSync(join(configDir, 'server-ops', 'audit.json'), 'utf8'))).toMatchObject({ version: 2 })
  })

  test('Given 损坏或未知版本文件 When 构造 Store Then 保留原文件并以稳定错误阻断 list 与 append', () => {
    for (const content of ['{broken', JSON.stringify({ version: 3, records: [] })]) {
      const configDir = createConfigDir()
  ...
  test('Given 合法 v2 主文件 When 启动 Store Then 复用已校验原始快照且不重复安全读取', () => {
    const configDir = createConfigDir()
    const directory = join(configDir, 'server-ops')
    mkdirSync(directory, { recursive: true })
    writeFileSync(join(directory, 'audit.json'), JSON.stringify({ version: 2, records: [] }))
    let readCount = 0
  ...
  test('Given v1 迁移原子写失败 When 后续读取或追加 Then Store 持续 fail closed', () => {
    const configDir = createConfigDir()
    const directory = join(configDir, 'server-ops')
    mkdirSync(directory, { recursive: true })
