...
describe('Design preload', () => {
  test('Given 固定 API When 逐一调用 Then 只透传对应 Design 通道和结构化参数', async () => {
    ...
    const calls: Array<[() => Promise<unknown>, string, unknown[]]> = [
      ...
      [() => api.createDesignJob({ projectId: 'p1', action: 'generate', prompt: 'x', position: { x: 0, y: 0 } }), DESIGN_IPC_CHANNELS.CREATE_JOB, [{ projectId: 'p1', action: 'generate', prompt: 'x', position: { x: 0, y: 0 } }]],
