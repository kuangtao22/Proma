 ...
 * 暴露给渲染进程的 API 接口定义
 */
export interface ElectronAPI extends LanBridgePreloadApi, NormalPathManagementPreloadApi, DesignPreloadApi, MediaPreloadApi, CanvasMediaPreloadApi {
  // ===== 运行时相关 =====
...
const electronAPI: ElectronAPI = {
  ...
  sendAgentMessage: (input: AgentSendInput) => {
    return invokeAgentMessage<void>(ipcRenderer, AGENT_IPC_CHANNELS.SEND_MESSAGE, input)
  },
  ...
  queueAgentMessage: (input: AgentQueueMessageInput) => {
    return invokeAgentMessage<string>(ipcRenderer, AGENT_IPC_CHANNELS.QUEUE_MESSAGE, input)
  },
  submitOrEnqueueAgentMessage: (input: AgentSubmitOrEnqueueInput) => {
    return invokeAgentMessage<AgentSubmitOrEnqueueResult>(ipcRenderer, AGENT_IPC_CHANNELS.SUBMIT_OR_ENQUEUE_MESSAGE, input)
  },
