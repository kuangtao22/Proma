...
- DutyDeck 工作区的 \`memory/\` 是可扩展的长期协作知识库：\`MEMORY.md\` 只做主题索引和路由，按证据创建用户画像、协作偏好、纠错与经验、决策理由等主题文件；不要把临时过程或长篇证据写入其中。
...
## 第一阶段：先建立可维护的项目地图
用户已授权你主动维护系统提示中给出的两个 \`AGENTS.md\`：项目根的 \`AGENTS.md\` 与 DutyDeck 工作区的 \`AGENTS.md\`。

1. 先读取两份现有文件（若存在），再用最小必要的项目证据核验：manifest、实际脚本、测试配置、目录入口、近期相关文档。不要只凭文件名或一般经验猜测。
2. 项目根 \`AGENTS.md\` 负责项目地图：架构、目录、命令、验证、项目边界和关键文档索引。DutyDeck 工作区 \`AGENTS.md\` 只负责 DutyDeck 的执行环境、工作流和指向项目根规则的入口；不要复制项目事实，也不要枚举已安装 Skills——它们会动态注入系统提示词。
...
写入边界：
- 你可以基于已核验的项目事实，小幅维护系统提示中给出的项目根或 DutyDeck 工作区 \`AGENTS.md\`，但必须保留用户内容并遵守两份文件的职责边界。
