...
function extractTurnUsage(turnMessages: SDKMessage[]): { durationMs?: number; usage?: AgentEventUsage } {
  for (const msg of turnMessages) {
    ...
    if (!u) return {
      durationMs,
      ...(resultMsg.usageStatus ? { usage: { usageStatus: resultMsg.usageStatus } } : {}),
    }
    ...
    return {
      durationMs,
      usage: {
        ...(resultMsg.usageStatus ? { usageStatus: resultMsg.usageStatus } : {}),
...
function ScheduledRunBadge(): React.ReactElement {
  ...
  return (
    <button
      ...
      title="来自 DutyDeck 定时任务，点击查看设置"
    >
      <Clock className="size-3" />
      <span>来自 DutyDeck 定时任务</span>
    </button>
  )
}
