...
export interface MediaRunProgressController {
  start(): void
  setJobs(jobs: readonly DesignJobRecord[]): void
  whenIdle(): Promise<void>
  dispose(): void
}

/** 判断 Job 是否需要统一媒体运行投影。 */
function isActiveComfyImageJob(job: DesignJobRecord): boolean {
  return job.target?.kind === 'canvas-image'
    && (job.status === 'queued' || job.status === 'running')
    && job.imageModelSnapshot?.executor === 'comfyui'
}
  ...
  return {
    ...
    setJobs: (jobs) => {
      ...
      desiredJobIds = new Set(jobs.filter((job) => (
        job.projectId === dependencies.projectId && isActiveComfyImageJob(job)
      )).map((job) => job.id))
  ...
  React.useEffect(() => {
    ...
    const controller = createMediaRunProgressController({
      ...
      onChange: (runs) => setProgress(new Map(
        [...runs].map(([jobId, run]) => [jobId, projectMediaRunProgress(run)]),
      )),
    })
    controllerRef.current = controller
    controller.start()
