...
function permissionMessage(permission: PlanningNativeSyncPermissionResult): string {
  switch (permission.status) {
    case 'not-determined': return `授权后选择一个 ${COPY[permission.entity].targetLabel}；DutyDeck 不会读取或导入其他系统项目。`
    case 'write-only': return '当前只有“仅写入”权限，无法列出可选目标。请升级为“完整访问权限”。'
    case 'denied': return '系统已拒绝访问。请在 macOS 系统设置中允许 DutyDeck 访问。'
...
export function PlanningNativeSyncControl({ entity }: { entity: PlanningNativeSyncEntity }): React.ReactElement | null {
  ...
  const disconnect = async (id: string): Promise<void> => {
    setSaving(true)
    try { await window.electronAPI.disconnectPlanningNativeConnection(id); await refresh(); toast.success('已从 DutyDeck 隐藏该系统集合') }
  ...
  const resolveConflict = async (id: string, resolution: 'keep_proma' | 'keep_system'): Promise<void> => {
    setSaving(true)
    try { await window.electronAPI.resolvePlanningNativeSyncConflict({ id, resolution }); await refresh(); toast.success(resolution === 'keep_proma' ? '将以 DutyDeck 版本覆盖系统项目' : '已保留系统版本') }
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    ...
    <PopoverContent align="start" className="w-80 space-y-3 p-3">
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">设置 DutyDeck 受管的{COPY[entity].label}；仅同步到你明确选择的一个目标。Calendar 会双向回流，Reminder 保持单向发布。</p>
        <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">建议先在系统中创建一个名为「DutyDeck」的{entity === 'calendar' ? '日历' : '提醒事项列表'}，再将它设为同步目标，避免与个人项目混杂。</p>
      </div>
