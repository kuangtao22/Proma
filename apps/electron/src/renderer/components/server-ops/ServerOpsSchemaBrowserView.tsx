...
import { ChevronLeft, ChevronRight, Columns3, Database, KeyRound, ListTree, RefreshCw, Search, Table2 } from 'lucide-react'
import type { ServerOpsDataSchemaCell, ServerOpsDataSchemaTableSummary } from '@proma/shared'
...
export function ServerOpsDatabaseSelector({ projection, onSelectDatabase, onRefresh, children, compact = false }: {
  ...
  const controls = <>
      ...
      <Select value={projection.database ?? ''} onValueChange={onSelectDatabase}>
        <SelectTrigger className={cn(compact ? 'h-7 min-w-28 max-w-56 flex-1 basis-36 text-xs' : 'h-8 min-w-32 max-w-xs flex-1 basis-40 text-xs')} aria-label="选择数据库"><SelectValue placeholder={projection.status === 'loading' && !projection.databases.length ? '正在读取数据库…' : '选择数据库'} /></SelectTrigger>
      ...
      <Button size="icon-sm" variant="ghost" aria-label="刷新数据库目录" disabled={projection.status === 'loading'} onClick={onRefresh}><RefreshCw className="size-3.5" /></Button>
      {children}
  </>
  ...
  if (compact) return <div className="flex min-w-0 flex-[1_1_14rem] flex-col" data-server-ops-database-selector data-server-ops-database-selector-compact="true"><div className="flex min-w-0 items-center gap-1.5">{controls}</div>{status}</div>
...
export function ServerOpsSchemaBrowserView({
  ...
  onRefreshTables = onRefresh, directoryWidth = 190, onDirectoryWidthChange, showDatabaseSelector = true,
}: ServerOpsSchemaBrowserViewProps): React.ReactElement {
  /** 临时界面状态不持久化业务内容。 */
  const [uiAtom] = React.useState(() => atom({ search: '', directoryOpen: false }))
  const [ui, setUi] = useAtom(uiAtom)
  /** 覆盖目录限制在发起工作区。 */
  const containerRef = React.useRef<HTMLDivElement>(null)
  /** 手势起点不触发 React 更新。 */
  const resizeRef = React.useRef<{ x: number; width: number } | null>(null)
  React.useEffect(() => { setUi({ search: '', directoryOpen: false }) }, [projection.sourceId, projection.database, setUi])
  /** 搜索只筛选已加载目录，不额外访问数据库。 */
  const filtered = projection.tables.filter((table) => table.name.toLocaleLowerCase().includes(ui.search.toLocaleLowerCase()))
  /** 属性复用同一份目录元数据。 */
  const selectedSummary = projection.tables.find((table) => table.name === projection.selectedTable)
  /** 选择后关闭覆盖层，Radix 恢复触发器焦点。 */
  const selectTable = (table: string): void => { onOpenTable(table); setUi((previous) => ({ ...previous, directoryOpen: false })) }
  /** 限制目录宽度，保障右侧最小可读空间。 */
  ...
  const directory = (
    <div className="flex h-full min-h-0 flex-col bg-muted/10" data-server-ops-schema-directory>
      ...
      <ul className="min-h-0 flex-1 space-y-0.5 overflow-y-auto px-2 py-1" data-server-ops-schema-table-list aria-label="表目录">
        {filtered.map((table) => <li key={table.name}>
      ...
      <div className="db-browser-body min-h-0 min-w-0 flex-1 overflow-hidden">
        ...
        {projection.selectedTable === null ? <div className="flex min-w-0 flex-1 flex-col items-center justify-center gap-3 p-6 text-center text-muted-foreground"><div className="rounded-xl bg-muted/50 p-3"><Table2 className="size-6" /></div><p className="text-sm font-medium text-foreground">{projection.database === null ? '选择数据库开始浏览' : '从表目录选择一张表'}</p><p className="text-xs">数据、结构、索引和属性会显示在这里</p></div> : (
          <Tabs className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden" value={projection.detailTab} onValueChange={(tab) => onDetailTabChange(tab as ServerOpsSchemaDetailTab)} data-server-ops-schema-table-detail={projection.selectedTable}>
            ...
            <TabsContent value="data" className="m-0 flex min-h-0 flex-1 flex-col overflow-hidden data-[state=inactive]:hidden"><SchemaRowsPanel state={projection.rows} onLoadRows={onLoadRows} onRetry={onRefresh} /></TabsContent>
...
/** 只读分页网格，空表也保留真实字段头。 */
function SchemaRowsPanel({ state, onLoadRows, onRetry }: { state: ServerOpsSchemaBrowserProjection['rows']; onLoadRows: (offset: number) => void; onRetry: () => void }): React.ReactElement {
  /** 在途请求与偏移上限共同限制分页操作。 */
  ...
  return <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden" data-server-ops-schema-rows>
    ...
    <div className="min-h-0 min-w-0 flex-1 overflow-auto"><table className={gridClass}><thead><tr>{state.columns.map((column) => <th key={column} scope="col">{column}</th>)}</tr></thead><tbody>{state.rows.map((row, rowIndex) => <tr key={`${state.offset + rowIndex}`}>{row.map((cell, cellIndex) => <td key={cellIndex} className={cn('max-w-[20rem] truncate font-mono', typeof cell === 'string' && /^-?\d+(\.\d+)?$/u.test(cell) && 'text-right tabular-nums', (cell === null || cell === '' || typeof cell === 'object') && 'text-muted-foreground italic')} title={formatServerOpsSchemaCell(cell)}>{formatServerOpsSchemaCell(cell)}</td>)}</tr>)}</tbody></table>{state.status === 'ready' && !state.rows.length ? <div className="p-8 text-center text-xs text-muted-foreground">暂无记录</div> : null}</div>
