...
export function summarizeServerOpsReadAccess(access: ServerOpsAgentReadAccess | null, now: number, names?: ReadonlyMap<string, string>): ServerOpsReadSummary {
  ...
  const databaseTargets = access.resources.flatMap((resource) => {
    ...
    const label = names?.get(`data:${resource.sourceId}`) || (resource.kind === 'postgresql' ? 'PostgreSQL' : resource.kind === 'mysql' ? 'MySQL' : 'SQLite')
    ...
    return [[label, ...((resource.kind === 'mysql' || resource.kind === 'postgresql') && resource.instance ? ['实例诊断'] : []), scopes.join(' / ')].filter(Boolean).join(' · ')]
  })
  ...
  const instanceCount = access.resources.filter((resource) => (resource.kind === 'mysql' || resource.kind === 'postgresql') && resource.instance).length
  const targets = [
    ...databaseTargets,
  ...
  const scopes = access.resources.flatMap((resource) => resource.kind === 'mysql' || resource.kind === 'postgresql' || resource.kind === 'sqlite' ? resource.databases : [])
  const capabilities = [
    ...(scopes.length ? ['结构'] : []),
