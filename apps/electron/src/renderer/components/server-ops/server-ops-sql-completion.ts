...
const MYSQL_KEYWORDS = new Set(MySQL.spec.keywords?.toLowerCase().split(/\s+/u))
...
function quoteIdentifier(value: string): string {
  return /^[\p{L}_$][\p{L}\p{N}_$]*$/u.test(value) && !MYSQL_KEYWORDS.has(value.toLowerCase())
...
function columnCompletion(column: ServerOpsDataSchemaColumn): Completion {
  return { label: column.name, type: 'property', detail: column.type,
    ...(column.comment ? { info: column.comment } : {}), apply: quoteIdentifier(column.name) }
}
...
export function createServerOpsSqlCompletionSource(options: ServerOpsSqlCompletionOptions): CompletionSource {
  ...
  return async (context): Promise<CompletionResult | null> => {
    ...
    for (const table of schema.tables) {
      ...
      namespace[table.name] = { self: { label: table.name, type: 'type', detail: '表', apply: quoteIdentifier(table.name), ...(table.comment ? { info: table.comment } : {}) }, children: getColumns(schema, table.name).map(columnCompletion) }
    }
    /** 内置 SQL source 负责别名限定与反引号转义。 */
    const native = await schemaCompletionSource({ dialect: MySQL, schema: namespace })(context)
    ...
    if (qualifier) return nativeOrUnicode ? { ...nativeOrUnicode, options: native?.options ?? (reference ? getColumns(schema, reference.table).map(columnCompletion) : []) } : null
    const keywordResult = await keywords(context)
    ...
    for (const source of references) {
      for (const column of getColumns(schema, source.table)) {
        const completion = columnCompletion(column)
        if ((counts.get(column.name) ?? 0) > 1) {
          completion.label = `${source.alias}.${column.name}`
          completion.apply = `${quoteIdentifier(source.alias)}.${quoteIdentifier(column.name)}`
        }
