...
/**
 * 将主进程 deferred 快照合并到 Renderer 队列投影。
 * @param current reload 窗口内可能已新建或更新的本地队列。
 * @param snapshots 主进程真实持有的 deferred 输入快照。
 * @returns 按快照顺序追加未知 ID 后的队列；不从 prompt 文本伪造展示数据。
 */
export function mergeRestoredAgentQueuedMessages(
  current: readonly AgentQueuedMessage[],
  snapshots: readonly AgentQueuedMessageSnapshot[],
): AgentQueuedMessage[] {
  /** 本地投影优先，避免 reload 窗口内的用户更新被旧快照覆盖。 */
  const knownIds = new Set(current.map((message) => message.id))
  /** 只从主进程明确保留的字段重建，不解析 rawUserMessage 中的标记。 */
  const recovered = snapshots
    ...
    .map(({ input, queuedAt }) => createAgentQueuedMessage(
      ...
      queuedAt,
      input.queuePresentation?.quotedSelection ?? null,
      {
        fileReferenceBlock: input.queuePresentation?.fileReferenceBlock,
        attachments: input.queuePresentation?.attachments,
...
export function applyAgentQueuedMessageStatus(store: Store, status: AgentQueuedMessageStatus): boolean {
  if (status.status === 'started') {
    return applyAgentQueuedMessageStartedStatus(store, status)
  }
...
export function useGlobalAgentListeners(): void {
  ...
  useEffect(() => {
    ...
    const cleanupQueuedMessageStatus = window.electronAPI.onAgentQueuedMessageStatus((status) => {
      ...
      if (applied && status.status === 'started') {
        promoteDelegatedSessionForRunStart(status.sessionId, status.startedAt)
        clearCompletionAttention(status.sessionId)
      }
    })
    ...
    const restoreQueuedMessages = async (): Promise<void> => {
      ...
      for (const sessionId of sessionIds) {
        ...
        store.set(agentSessionMessageQueueAtom, (previous) => {
          ...
          const recovered = mergeRestoredAgentQueuedMessages(current, snapshots)
          if (recovered.length === current.length) return previous
          const next = new Map(previous)
          next.set(sessionId, recovered)
          return next
        })
      }
    }
