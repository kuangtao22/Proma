...
// Canvas Agent 需要等待完整模型终态；与 Task 8 的工作流总时限保持一致。
export const CANVAS_AGENT_TOOL_TIMEOUT_MS = 15 * 60_000

/**
 * Utility Process 请求主进程的等待时间。
 * AskUserQuestion、Canvas Agent 与可信 Design 图片工具分别使用独立长时限，其他能力保持默认故障检测。
 */
export function getParentRequestTimeoutMs(method: string, payload: unknown): number {
  ...
  if (
    method === AGENT_RUNTIME_METHODS.CAPABILITY_CUSTOM_TOOL
    && toolName === CANVAS_AGENT_TOOL
  ) {
    return CANVAS_AGENT_TOOL_TIMEOUT_MS
  }
