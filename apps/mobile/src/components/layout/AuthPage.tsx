...
export function AuthPage({
  ...
  }, [pin, host, port, onSuccess, onManualAttempt])
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full px-6 bg-background" style={{ paddingTop: 'var(--safe-t)', paddingBottom: 'var(--safe-b)' }}>
      <div className="w-full max-w-sm">
        ...
        {pairingPending && (
          <div className="mb-4 rounded-lg border border-border bg-muted/50 px-3 py-2.5 text-center text-sm text-foreground" role="status">
            正在验证扫码连接...
          </div>
        )}
        {pairingError && (
          <div className="mb-4 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2.5 text-center text-xs text-destructive" role="alert">
            {pairingError}
          </div>
        )}
