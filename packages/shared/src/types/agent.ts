...
/** Agent 普通发送输入允许的完整字段集合。 */
const AGENT_SEND_INPUT_KEYS = [
  'sessionId', 'userMessage', 'rawUserMessage', 'userMessageUuid', 'channelId', 'modelId',
  'workspaceId', 'additionalDirectories', 'permissionModeOverride', 'mentionedSkills',
  'mentionedMcpServers', 'mentionedSessionIds', 'mentionedTodoIds', 'mentionedCalendarEventIds',
  'startedAt', 'retryOfErrorUuid', 'triggeredBy', 'automationContext', 'canvasNodeReferences',
] as const

/** Agent 活跃队列输入允许的完整字段集合。 */
const AGENT_QUEUE_MESSAGE_INPUT_KEYS = [
  'sessionId', 'userMessage', 'rawUserMessage', 'uuid', 'interrupt', 'mentionedSkills',
  'mentionedMcpServers', 'mentionedSessionIds', 'mentionedTodoIds', 'mentionedCalendarEventIds',
  'canvasNodeReferences',
] as const

/** 判断未知值是否为不含未知字段且具备全部必填字段的普通记录。 */
function hasOnlyAgentInputKeys(
  value: unknown,
...
/** 严格复制可选字符串数组，避免 IPC 调用方后续修改原数组。 */
function parseOptionalAgentStringList(value: unknown, errorCode: string): string[] | undefined {
...
/** 判断可选值是否为字符串。 */
function isOptionalAgentString(value: unknown): value is string | undefined {
  return value === undefined || typeof value === 'string'
}

/** 判断可选时间戳是否为非负安全整数。 */
function isOptionalAgentTimestamp(value: unknown): value is number | undefined {
  return value === undefined || (typeof value === 'number' && Number.isSafeInteger(value) && value >= 0)
}

/** 严格解析可选 Canvas 节点引用数组。 */
function parseOptionalCanvasNodeReferences(value: unknown): CanvasNodeReference[] | undefined {
  return value === undefined ? undefined : parseCanvasNodeReferences(value)
}

/**
 * 严格解析 Agent 普通发送字段，并允许派生合同声明自己的额外字段。
 * @param value 待解析的 Renderer 或 Bridge 输入。
 * @param extraAllowedKeys deferred/submit 合同额外允许的字段。
 * @returns 不含派生字段且数组均已复制的普通发送输入。
 */
function parseAgentSendRecord(
  value: unknown,
  extraAllowedKeys: readonly string[] = [],
): AgentSendInput {
  /** 派生合同在普通发送字段之外允许的字段集合。 */
  const allowedKeys = [...AGENT_SEND_INPUT_KEYS, ...extraAllowedKeys]
  if (!hasOnlyAgentInputKeys(value, allowedKeys, ['sessionId', 'userMessage', 'channelId'])
    || typeof value.sessionId !== 'string'
    || typeof value.userMessage !== 'string'
    || typeof value.channelId !== 'string'
    || !isOptionalAgentString(value.rawUserMessage)
    || !isOptionalAgentString(value.userMessageUuid)
    || !isOptionalAgentString(value.modelId)
    || !isOptionalAgentString(value.workspaceId)
    || !isOptionalAgentString(value.retryOfErrorUuid)
    || !isOptionalAgentString(value.automationContext)
    || (value.permissionModeOverride !== undefined
      && value.permissionModeOverride !== 'bypassPermissions'
      && value.permissionModeOverride !== 'plan')
    || !isOptionalAgentTimestamp(value.startedAt)
    || (value.triggeredBy !== undefined
      && value.triggeredBy !== 'user'
      && value.triggeredBy !== 'automation'
      && value.triggeredBy !== 'delegation'
      && value.triggeredBy !== 'external')) {
    throw new Error('AGENT_SEND_INPUT_INVALID')
  }
  /** 各目录与 mention 列表在边界处独立复制。 */
  const additionalDirectories = parseOptionalAgentStringList(value.additionalDirectories, 'AGENT_SEND_INPUT_INVALID')
  /** 显式 Skill 引用列表。 */
  const mentionedSkills = parseOptionalAgentStringList(value.mentionedSkills, 'AGENT_SEND_INPUT_INVALID')
  /** 显式 MCP 引用列表。 */
  const mentionedMcpServers = parseOptionalAgentStringList(value.mentionedMcpServers, 'AGENT_SEND_INPUT_INVALID')
  /** 显式会话引用列表。 */
  const mentionedSessionIds = parseOptionalAgentStringList(value.mentionedSessionIds, 'AGENT_SEND_INPUT_INVALID')
  /** 显式 Todo 引用列表。 */
  const mentionedTodoIds = parseOptionalAgentStringList(value.mentionedTodoIds, 'AGENT_SEND_INPUT_INVALID')
  /** 显式日程引用列表。 */
  const mentionedCalendarEventIds = parseOptionalAgentStringList(value.mentionedCalendarEventIds, 'AGENT_SEND_INPUT_INVALID')
  /** Canvas 节点引用与 mention、附件保持独立。 */
  const canvasNodeReferences = parseOptionalCanvasNodeReferences(value.canvasNodeReferences)
  return {
    sessionId: value.sessionId,
    userMessage: value.userMessage,
    channelId: value.channelId,
    ...(value.rawUserMessage === undefined ? {} : { rawUserMessage: value.rawUserMessage }),
    ...(value.userMessageUuid === undefined ? {} : { userMessageUuid: value.userMessageUuid }),
    ...(value.modelId === undefined ? {} : { modelId: value.modelId }),
    ...(value.workspaceId === undefined ? {} : { workspaceId: value.workspaceId }),
    ...(additionalDirectories === undefined ? {} : { additionalDirectories }),
    ...(value.permissionModeOverride === undefined ? {} : { permissionModeOverride: value.permissionModeOverride }),
    ...
    ...(value.retryOfErrorUuid === undefined ? {} : { retryOfErrorUuid: value.retryOfErrorUuid }),
    ...(value.triggeredBy === undefined ? {} : { triggeredBy: value.triggeredBy }),
    ...(value.automationContext === undefined ? {} : { automationContext: value.automationContext }),
    ...(canvasNodeReferences === undefined ? {} : { canvasNodeReferences }),
  }
}

/** 严格解析普通 Agent 发送输入。 */
export function parseAgentSendInput(value: unknown): AgentSendInput {
  return parseAgentSendRecord(value)
}

/** 严格解析等待当前 run 结束后发送的 deferred 消息。 */
export function parseAgentDeferredQueueMessageInput(value: unknown): AgentDeferredQueueMessageInput {
  /** 普通发送字段先在允许 queueMessageId 的严格集合内解析。 */
  const input = parseAgentSendRecord(value, ['queueMessageId'])
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('AGENT_DEFERRED_QUEUE_MESSAGE_INPUT_INVALID')
  }
  /** deferred 控制字段通过普通记录读取。 */
  const control = value as Record<string, unknown>
  if (typeof control.queueMessageId !== 'string') {
    throw new Error('AGENT_DEFERRED_QUEUE_MESSAGE_INPUT_INVALID')
  }
  /** 已验证的稳定队列消息身份。 */
  const queueMessageId = control.queueMessageId
  return { ...input, queueMessageId }
}

/** 严格解析原子提交或排队输入。 */
export function parseAgentSubmitOrEnqueueInput(value: unknown): AgentSubmitOrEnqueueInput {
  /** 普通发送字段先在允许提交控制字段的严格集合内解析。 */
  const input = parseAgentSendRecord(value, ['queueMessageId', 'dispatch', 'interrupt'])
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('AGENT_SUBMIT_OR_ENQUEUE_INPUT_INVALID')
  }
  /** 提交控制字段通过普通记录读取。 */
  const control = value as Record<string, unknown>
  if (typeof control.queueMessageId !== 'string'
    || (control.dispatch !== 'after_current' && control.dispatch !== 'now')
    || (control.interrupt !== undefined && typeof control.interrupt !== 'boolean')) {
    throw new Error('AGENT_SUBMIT_OR_ENQUEUE_INPUT_INVALID')
  }
  return {
    ...input,
    queueMessageId: control.queueMessageId,
    dispatch: control.dispatch,
    ...(control.interrupt === undefined ? {} : { interrupt: control.interrupt }),
  }
}

/** 严格解析向活跃 Agent 注入的队列消息。 */
export function parseAgentQueueMessageInput(value: unknown): AgentQueueMessageInput {
  if (!hasOnlyAgentInputKeys(value, AGENT_QUEUE_MESSAGE_INPUT_KEYS, ['sessionId', 'userMessage'])
    || typeof value.sessionId !== 'string'
    || typeof value.userMessage !== 'string'
    || !isOptionalAgentString(value.rawUserMessage)
    || !isOptionalAgentString(value.uuid)
    || (value.interrupt !== undefined && typeof value.interrupt !== 'boolean')) {
    throw new Error('AGENT_QUEUE_MESSAGE_INPUT_INVALID')
  }
  /** 显式 Skill 引用列表。 */
  const mentionedSkills = parseOptionalAgentStringList(value.mentionedSkills, 'AGENT_QUEUE_MESSAGE_INPUT_INVALID')
  /** 显式 MCP 引用列表。 */
  const mentionedMcpServers = parseOptionalAgentStringList(value.mentionedMcpServers, 'AGENT_QUEUE_MESSAGE_INPUT_INVALID')
  /** 显式会话引用列表。 */
  const mentionedSessionIds = parseOptionalAgentStringList(value.mentionedSessionIds, 'AGENT_QUEUE_MESSAGE_INPUT_INVALID')
  /** 显式 Todo 引用列表。 */
  const mentionedTodoIds = parseOptionalAgentStringList(value.mentionedTodoIds, 'AGENT_QUEUE_MESSAGE_INPUT_INVALID')
  /** 显式日程引用列表。 */
  const mentionedCalendarEventIds = parseOptionalAgentStringList(value.mentionedCalendarEventIds, 'AGENT_QUEUE_MESSAGE_INPUT_INVALID')
  /** Canvas 节点引用数组严格解析且独立复制。 */
  const canvasNodeReferences = parseOptionalCanvasNodeReferences(value.canvasNodeReferences)
  return {
    sessionId: value.sessionId,
    userMessage: value.userMessage,
